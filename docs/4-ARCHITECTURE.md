...
该域不新增运行时服务单元，仍落在现有应用和服务模块内。所有公开能力默认关闭，正式开放前必须同时满足产品、合规和运营开关；写路径必须额外经过权限与合规守门。
...
- CLI；
- Agent-friendly 接口；
- 浏览器扩展 SDK（公开站仅用于扩展提交；MultiPost 多平台发布入口仅在管理后台开放）；
- API Key；
- 外部调用限流；
- 开放接口配额；
- 公共内容与授权内容的调用边界。
...
浏览器扩展属于开放调用层的端侧延伸：网页只通过受控 SDK 与扩展通信。用户侧后端只暴露明确鉴权的扩展提交接口；MultiPost 多平台发布属于管理后台运营能力，由 admin-api 的发布任务队列承接，不向普通读者开放。扩展提交内容必须绑定已有 source，或由服务端按来源 host 自动匹配；仍未命中时落到用户私有 extension inbox 源，避免内容资产脱离来源模型。
...
├── bestblogs-mobile         # 移动端 App（当前暂缓）
├── dujian-app               # 读见：后续国内部署版本
├── bestblogs-service
...
bestblogs-mobile 是移动端 App 模块，目标是承载移动阅读、早报触达、内容浏览、AI 伴读和推送。当前 Mobile 项目和相关功能暂时搁置，近期优先验证 PMF 与用户需求，避免在核心价值尚未完全验证前扩张端侧复杂度。
...
dujian-app 是「读见」国内部署版本，用于承接 BestBlogs 后续在国内访问、本地化和合规部署场景中的轻量 Web 形态。
...
- 国内部署环境下的公开页面；
- 必要的登录与账户设置入口；
- 与 BestBlogs 主服务共享产品语义和内容边界。

边界：

- dujian-app 不复制 BestBlogs 的核心业务真相；
- 不作为新的产品线独立演化；
- 不绕过主服务的鉴权、Pro、内容可见性和安全规则；
- 国内部署细节、服务器配置和备案操作不进入公开架构文档。
...
Mobile 是私人阅读助手的重要触达端，但当前暂缓。重启 Mobile 投入前需要先看到 PMF 和用户需求信号，而不是为了多端完整性提前扩张。

需要重点评估：

1. 用户是否已经稳定使用 Web / 邮件中的早报工作流；
2. 主题早报是否形成足够明确的移动端使用场景；
3. Mobile 是否强化我的早报、主题早报、我的阅读和我的回顾，而不是制造新的复杂入口；
4. Push 是否提升价值触达，而不是变成骚扰；
5. 移动端 Pro 状态是否能与 Web 保持一致；
6. 移动端是否复制了本应在服务端治理的业务规则；
7. 多端 UI 差异是否仍保持统一产品语义。
...
| 我的早报生成 | UserBriefJob | Pro 个性化早报生成，支持多节点下的任务协调 |
| Pro 即时早报 | ProBriefBootstrapJob / ProBriefBootstrapService | Pro 用户开通后的首份早报生成链路 |
| 邮件投递可观测 | MessageDeliveryLogService | Brief / Daily Review / Newsletter 的投递状态与重试锚点 |
| 内容多渠道发布 | PublishTaskService / ApprovalTicket | 多渠道发布任务与人工审批锚点 |
| 浏览器扩展提交 | Extension submit API | 用户侧扩展提交入库，服务端按来源关系归属 |
| 管理端多平台发布 | Multipost task API | 管理后台运营使用的多平台发布任务队列 |
| URL 一键发现订阅源 | SourceDiscoveryRegistry + SourcePlatformDiscoveryAdapter | 多平台来源探测与通用 HTML 兜底 |
